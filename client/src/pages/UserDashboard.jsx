...
const UserDashboard = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 mt-6">
      ...
      {/* Welcome Hero Card */}
      <div className="bg-slate-900/60 backdrop-blur-xl rounded-3xl p-6 sm:p-8 mb-8 border border-slate-800/80 flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-4 sm:gap-6 relative overflow-hidden shadow-2xl">
        ...
        <div className="relative group">
          <div className="w-20 h-20 bg-gradient-to-tr from-emerald-400 to-teal-600 text-white rounded-full flex items-center justify-center text-3xl font-black shadow-xl border-2 border-emerald-400/20 group-hover:scale-102 transition duration-300 select-none uppercase">
        ...
        <div className="flex flex-col items-center sm:items-start justify-center">
          ...
          <p className="text-slate-400 text-sm font-bold flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]"></span> User Account Dashboard
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-8 px-1">
        <h2 className="text-xl sm:text-2xl font-black text-white flex items-center gap-2 sm:gap-3">
          <FaTicketAlt className="text-emerald-400 rotate-[-10deg]" /> My Booking Requests
        </h2>
      </div>

      {bookings.length === 0 ? (
        <div className="bg-slate-900/40 backdrop-blur-md rounded-2xl p-12 text-center border border-slate-850 shadow-xl">
          <div className="w-20 h-20 bg-slate-950 border border-slate-800/80 rounded-full flex items-center justify-center mx-auto mb-4 text-emerald-400 text-3xl">
            <FaTicketAlt />
          </div>
          <p className="text-xl text-slate-400 mb-6 mt-4 font-semibold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {bookings.map((booking) => (
            <div
              key={booking._id}
              className="bg-slate-900/50 backdrop-blur-sm rounded-2xl overflow-hidden border border-slate-850 hover:border-slate-800 shadow-lg flex flex-col relative min-h-[300px]"
            >
              {booking.eventId && booking.status === "confirmed" ? (
                ...
                <div className="relative bg-gradient-to-br from-slate-950 via-slate-950 to-slate-900 text-white p-6 flex flex-col justify-between flex-grow rounded-2xl overflow-hidden select-none border border-slate-800/80 shadow-2xl">
                  {/* Left & Right ticket cutouts - matches deep slate-950 background */}
                  <div className="absolute -left-2 top-1/2 -translate-y-1/2 w-4 h-8 bg-slate-950 rounded-r-full shadow-inner border-r border-slate-800"></div>
                  <div className="absolute -right-2 top-1/2 -translate-y-1/2 w-4 h-8 bg-slate-950 rounded-l-full shadow-inner border-l border-slate-800"></div>
                ...
                <>
                  <div className="p-6 flex-grow flex flex-col justify-between">
                    {booking.eventId ? (
                      <>
                        ...
                        <div className="text-slate-400 space-y-2 mt-auto">
                          ...
                          <div className="flex items-center gap-2 text-xs font-bold text-white">
                            <FaPassIcon className="text-emerald-400" />
                  ...
                  <div className="p-4 bg-slate-950/70 flex justify-between items-center shrink-0 border-t border-slate-850 w-full rounded-b-2xl">
                    {booking.eventId && booking.status !== "cancelled" ? (
                      <>
                        ...
                        <button
                          ...
                          className="text-rose-400 hover:text-rose-300 font-bold text-xs transition flex items-center gap-1 cursor-pointer"
                        >
                          <FaTimesCircle /> Cancel
                        </button>
                      </>
                    ) : (
                      <div className="w-full text-center text-xs text-slate-500 italic font-bold uppercase tracking-wider py-1">
                        Cancelled
                      </div>
                    )}
                  </div>
